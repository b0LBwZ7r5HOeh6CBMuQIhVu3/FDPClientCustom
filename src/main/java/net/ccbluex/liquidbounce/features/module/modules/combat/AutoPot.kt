        ...
        when (motionEvent.eventState) {
            PRE -> {
                ...
                if (mc.thePlayer.health <= healthValue.get() && potionInHotbar != -1) {
                    ...
                    if (mc.thePlayer.posY - (collisionBlock?.y ?: 0).toFloat() >= groundDistanceValue.get())
                        return
                    ...
                    if (mc.thePlayer.rotationPitch <= 80F) {
                        RotationUtils.setTargetRotation(Rotation(RotationUtils.serverRotation.yaw+175, RandomUtils.nextFloat(80F, 90F)))
                    }
                    return
                }
