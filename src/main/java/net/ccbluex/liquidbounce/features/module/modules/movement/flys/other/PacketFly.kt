...
class PacketFly : FlyMode("PacketFly") {
    ...
    private val packetValue = ListValue("${valuePrefix}Packet", arrayOf("OldRedesky", "Jitter","SkyTp"), "Jitter")
    ...
    override fun onPacket(event: PacketEvent) {
        ...
        if (p is C03PacketPlayer && enabledTicks % 2 == 0) {
            when (packetValue.get().lowercase()) {
                ...
                "skytp" -> {
                    PacketUtils.sendPacketNoEvent(C06PacketPlayerPosLook(mc.thePlayer.posX , mc.thePlayer.posY+1024, mc.thePlayer.posZ , mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch, mc.thePlayer.posY % 0.125 == 0.0))
                }
            }
        }
    ...
    override fun onUpdate(event: UpdateEvent) {
        ...
        if (disablerValue.get()) {
            if (enabledTicks == 6) {
                LiquidBounce.hud.addNotification(Notification("PacketFly", "exploited You can fly freely until you disable fly.", NotifyType.WARNING, 2000))
            }
            if (enabledTicks <= 2) {
            ...
            if (mc.thePlayer.posY - player!!.posY > 10.0) {
                if (enabledTicks % 20 == 0) {
                    ...
                    LiquidBounce.hud.addNotification(Notification("PacketFly", "You got lag backed! Please land on the ground if it keeps happening.", NotifyType.WARNING, 2000))
                }
                return
            }
